...
static const uint16_t table[256] =
{
	//x0  |  x1  |  x2  |  x3  |  x4  |  x5  |  x6  |  x7  |  x8  |  x9  |  xA  |  xB  |  xC  |  xD  |  xE  |  xF
	 ...
	 none , none , error, error, none , none , error, error, none , none , none , none , none , none ,  rm  ,  rm  , // Fx
};
...
void Inst_x86::internal_decode(const std::string& buffer)
{
...
void Inst_x86::decode_prefixes(const std::string& buffer)
{
...
void Inst_x86::decode_opcode(const std::string& buffer)
{
...
void Inst_x86::read_disp(const std::string& buffer)
{
	disp = 0;

	for (int32_t i = 0; i < disp_size; ++i)
		disp |= buffer.at(ip + length+i) << i*8;
...
void Inst_x86::read_imm(const std::string& buffer)
{
	if (flags & op::am)
	{
		...
		has_imm = true;
		imm_size = (prefixes[3] == Prefix::p67) ? 2 : 4;
	}
	else
	{
		if (flags & op::i32)
		{
			has_imm = true;
			imm_size = (prefixes[2] == Prefix::p66) ? 2 : 4;
		}
	...
	if (has_imm)
	{
		...
		if (has_imm2)
		{
			...
			for (int32_t i = 0; i < imm2_size; ++i)
				imm2 |= buffer.at(ip + length+i) << i*8;
